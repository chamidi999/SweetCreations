...
type Toastype = "default" | "destructive"
...
type Action =
  ...
  | { type: typeof actionTypes.UPDATE_TOAST; toast: Partial<ToastProps> }
...
function toast({ ...props }: Omit<ToastProps, "id">) {
  const id = generateId()
  const update = (props: Partial<ToastProps>) => dispatch({ type: actionTypes.UPDATE_TOAST, toast: { ...props, id } })
  const dismiss = () => dispatch({ type: actionTypes.DISMISS_TOAST, toastId: id })
  dispatch({ type: actionTypes.ADD_TOAST, toast: { ...props, id, open: true, onOpenChange: (open: boolean) => !open && dismiss() } as any })
